...
router.post("/login", async (req, res) => {
  ...
  else res.json({ status: "found", logedAdmin: response });
});
...
router.put("/change-pwd", async (req, res) => {
...
router.post(
  ...
  async (req, res, next) => {
    ...
    const Photo = url + "/public/" + req.file.filename;
    await Admin.updateOne({ $set: { profileImage: Photo } })
      .then((vals) => res.send(val))
      .catch((err) => res.send(err));
  }
);
...
router.put("/updateOfficers/:id", upload.single("photo"), async (req, res) => {
  const { officerName, photo, email } = req.body;
  const id = req.params.id;
...
router.get("/numVisitor", async (req, res) => {
  ...
  await Visitor.find({}, { numberVisitor: 1, perPage: 1, perMonth: 1 })
    .then((val) => {
      perPageV = val[0]["perPage"];
      VisitorNum = val[0]["numberVisitor"];
      perMonthV = val[1]["perMonth"];
...
router.post("/updateNumVisitor/:id", async (req, res) => {
  ...
  // console.log(a.perPage[b],perPageVisitor);
  ...
  await Visitor.findOneAndUpdate(
    ...
    .then((val) => {
      console.log(
        `updated visitor count for month ${monthNow} : ${val.perMonth.count}`
      );
    })
    .catch((err) => console.log(err));
